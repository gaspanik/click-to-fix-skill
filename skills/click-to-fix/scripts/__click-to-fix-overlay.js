function injectClickToFix() {
  ...
  overlay.addEventListener('click', (e) => {
    ...
    if (top + 160 > window.innerHeight) top = clickY - 160;
    popup.style.left = left + 'px';
    popup.style.top = top + 'px';
    popup.style.display = 'block';
    document.getElementById('__ctf-coords').textContent = ` (${clickX}, ${clickY})`;
  ...
  sendBtn.addEventListener('click', () => {
    ...
    const elDesc = clickEl
      ...
      : '';
    const payload = { instruction: text, x: clickX, y: clickY, element: elDesc, timestamp: new Date().toISOString() };
